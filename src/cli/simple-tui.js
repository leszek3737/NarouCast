...
import { TRANSLATOR_PROVIDERS, TTS_PROVIDERS, AVAILABLE_VOICES } from '../tui/utils/voices.js';
  ...
  if (answers.ttsProvider !== 'none') {
    const voiceQuestion = {
      ...
      choices: AVAILABLE_VOICES
        .filter(v => v.provider === answers.ttsProvider)
        .map(v => ({ 
    ...
    const speedQuestion = {
      ...
      validate: (value) => value >= 0.25 && value <= 4.0 || 'Prędkość musi być między 0.25 a 4.0'
    };
    ...
    if (!defaultGoogleCreds) {
      try {
        ...
        if (jsonFiles.length === 1) {
          ...
          console.log(`📁 Automatycznie wykryto plik credentials: ${defaultGoogleCreds}`);
        }
      } catch (error) {
        // Ignore errors
      }
    ...
    const ttsAnswers = await inquirer.prompt([voiceQuestion, speedQuestion, audioDirQuestion]);
  ...
  const remainingQuestions = [
    {
      ...
      message: 'Ścieżka do pliku credentials Google (GOOGLE_APPLICATION_CREDENTIALS):',
      default: config?.google?.credentialsPath || '/tmp/google-service-account.json',
      when: (allAnswers) => answers.ttsProvider === 'google'
    },
    {
      type: 'input',
      name: 'outputDir',

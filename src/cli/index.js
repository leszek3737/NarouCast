...
class SyosetuTranslatorApp {
  constructor(options = {}) {
    ...
    this.options = {
      ...
      chapterDelay: options.chapterDelay || parseInt(process.env.CHAPTER_DELAY) || 3,
  ...
  async initializeComponents() {
    ...
    this.options = {
      ...this.options,
      translator: this.options.translator === 'deepseek' && savedConfig.translator?.provider 
        ? savedConfig.translator.provider 
        : this.options.translator,
      apiKey: !this.options.apiKey && savedConfig.translator?.apiKey 
        ? savedConfig.translator.apiKey 
        : this.options.apiKey,
      googleApiKey: !this.options.googleApiKey && savedConfig.translator?.apiKey && savedConfig.translator?.provider === 'google'
        ? savedConfig.translator.apiKey 
        : this.options.googleApiKey,
      openaiApiKey: !this.options.openaiApiKey && savedConfig.translator?.apiKey && savedConfig.translator?.provider === 'openai'
        ? savedConfig.translator.apiKey 
        : this.options.openaiApiKey,
      outputDir: this.options.outputDir === './output' && savedConfig.output?.directory
        ? savedConfig.output.directory
        : this.options.outputDir,
      autoContinue: this.options.autoContinue && savedConfig.general?.autoContinue !== undefined
        ? savedConfig.general.autoContinue
        : this.options.autoContinue,
      chapterDelay: this.options.chapterDelay === 3 && savedConfig.general?.chapterDelay
        ? savedConfig.general.chapterDelay
        : this.options.chapterDelay,
      tts: this.options.tts === 'none' && savedConfig.tts?.provider
        ? savedConfig.tts.provider
        : this.options.tts,
      voice: !this.options.voice && savedConfig.tts?.voice
        ? savedConfig.tts.voice
        : this.options.voice,
      audioDir: this.options.audioDir === './audio' && savedConfig.output?.audioDirectory
        ? savedConfig.output.audioDirectory
        : this.options.audioDir,
      ttsSpeed: this.options.ttsSpeed === 1.0 && savedConfig.tts?.speed
        ? savedConfig.tts.speed
        : this.options.ttsSpeed,
      googleCredentials: !this.options.googleCredentials && savedConfig.google?.credentialsPath
    ...
    if (this.options.googleCredentials) {
      process.env.GOOGLE_APPLICATION_CREDENTIALS = this.options.googleCredentials;
    }
  ...
  loadConfig() {
    try {
      ...
      console.warn('Nie można załadować config.json, używam wartości domyślnych');
      return {
        deepseek: {},
        scraping: {},
        navigation: {},
  ...
  async processChapter(url) {
    ...
    console.log(`📝 Seria: ${parsedUrl.seriesId}, Rozdział: ${parsedUrl.chapterNumber}`);
  ...
  async run(url) {
    try {
      ...
      console.log(`🔊 TTS: ${this.options.tts}${this.options.voice ? ` (głos: ${this.options.voice})` : ''}`);
...
program
  ...
  .option('--no-auto-continue', 'Zatrzymaj po każdym rozdziale i czekaj na potwierdzenie')
  ...
  .option('--translator <type>', 'Typ translatora: deepseek, google, openai', 'deepseek')
  .option('--tts <provider>', 'Provider TTS: openai, google, none', 'none')
  .option('--voice <voice>', 'Wybór głosu TTS (np. alloy, nova, pl-PL-Wavenet-A)')
  .option('--audio-dir <dir>', 'Katalog dla plików audio', './audio')
  .option('--speed <speed>', 'Szybkość czytania (0.25 - 4.0)', '1.0')
  .option('--max-chapters <number>', 'Maksymalna liczba rozdziałów do przetworzenia', '1000')

  ...
  splitTextIntoChunks(text, maxChunkSize) {
    ...
    for (const sentence of sentences) {
      ...
      if (Buffer.byteLength(sentenceWithPunctuation, 'utf8') > 900) {
        const splitSentences = this.splitLongSentence(sentenceWithPunctuation, 900);
        
        for (const splitSentence of splitSentences) {
          const testChunk = currentChunk ? `${currentChunk} ${splitSentence}` : splitSentence;
        ...
        const testChunk = currentChunk ? `${currentChunk} ${sentenceWithPunctuation}` : sentenceWithPunctuation;
        if (Buffer.byteLength(testChunk, 'utf8') <= maxChunkSize) {
          currentChunk = testChunk;
        } else {
          if (currentChunk) {

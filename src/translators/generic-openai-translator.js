...
export class GenericOpenAITranslator {
  ...
  async translateChapter(title, content) {
    try {
      console.log(`Tłumaczenie rozdziału za pomocą ${this.config.providerName}...`);
      ...
      throw new Error(`Błąd tłumaczenia ${this.config.providerName}: ${error.message}`);
    }
  }
...
Zwróć tylko przetłumaczony tekst bez żadnych dodatkowych komentarzy.`;
  ...
  static validateApiKey(apiKey) {
    if (!apiKey || typeof apiKey !== 'string') {
      throw new Error('Brak lub nieprawidłowy klucz API');
    }
